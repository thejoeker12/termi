...
# Executed as a script: reads the cache at the default XDG location
sandbox
mkdir -p "$XDG_CACHE_HOME/termi"
print -r -- "only-quote" > "$XDG_CACHE_HOME/termi/quotes.txt"
out=$("$REPO_ROOT/bin/termi"); st=$?
assert_eq "only-quote" "$out" "prints quote from the XDG cache path"
assert_eq 0 "$st" "exits 0 on success"

# Nothing fetched yet: silent success (must never break shell startup)
sandbox
out=$("$REPO_ROOT/bin/termi"); st=$?
assert_eq 0 "$st" "missing cache exits 0"
assert_eq "" "$out" "missing cache prints nothing"
